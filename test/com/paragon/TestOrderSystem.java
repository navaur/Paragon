...
//    }
    ...
    @Test
    public void confirmOrderWithinTenMinutesTest() {
        ...
        long time = quotes.get(id).timestamp + (10 * 60 * 1000);

        Order order = orderSystem.confirmOrder(id,userAuthToken,time);

        BigDecimal processingCharges = order.totalPrice.subtract(offers.get(0).price.multiply(OrderSystem.CASE_SIZE));

        BigDecimal varProcessingCharges = offers.get(0).price.multiply(OrderSystem.CASE_SIZE).multiply(DealProcessingChargeCalc.TIER2_PROCESSING_CHARGE_PERC) ;
